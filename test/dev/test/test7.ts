...
describe('My Action Test Suite', function () {
  ...
  async function startConversation() {
    ...
    await test.sendQuery('test');
    const resp = test.getLatestResponse();
    chai.expect(test.getSpeech()).to.equal("je n'ai pas compris. Que voulez-vous dire ?");
    test.assertSpeech("je n'ai pas compris. Que voulez-vous dire ?");

    let media = test.getMedia();
    chai.expect(media).to.equal(undefined);
